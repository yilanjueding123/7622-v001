...
static INT8U	g_led_flicker_state;	//0=Í¬Ê±ÉÁË¸	1=½»ÌæÉÁË¸
...
void ap_peripheral_B_key_exe(INT16U *tick_cnt_ptr);
void ap_peripheral_C_key_exe(INT16U *tick_cnt_ptr);
void ap_peripheral_D_key_exe(INT16U *tick_cnt_ptr);
...
void LED_pin_init(void)
{
	...
	/* add begin by Zhibo, 2017-04-28 é©¬è¾¾éœ‡åŠ¨æŽ§åˆ¶*/
	gpio_init_io(MOTOR_VIBRATION, GPIO_OUTPUT);
    gpio_set_port_attribute(MOTOR_VIBRATION, ATTRIBUTE_HIGH);
	gpio_write_io(MOTOR_VIBRATION, DATA_LOW);
...
void moto_vibration_on(void)
{
	gpio_write_io(MOTOR_VIBRATION, DATA_HIGH);
}
void moto_vibration_off(void)
{
	gpio_write_io(MOTOR_VIBRATION, DATA_LOW);
}
...
//        key_map[1].key_long = 0;
		/* add begin by Zhibo, 2017-04-28 */
		key_map[1].key_io = FUN_KEY_B;
		key_map[1].key_function = (KEYFUNC) ap_peripheral_B_key_exe;
		key_map[1].key_active = 1;
		key_map[1].key_long = 0;
		
		key_map[2].key_io = FUN_KEY_C;
		key_map[2].key_function = (KEYFUNC) ap_peripheral_C_key_exe;
		key_map[2].key_active = 1;
		key_map[2].key_long = 0;
		
		key_map[3].key_io = FUN_KEY_D;
		key_map[3].key_function = (KEYFUNC) ap_peripheral_D_key_exe;
		key_map[3].key_active = 1;
    ...
    for (i = 0 ; i < USE_IOKEY_NO ; i++)
    {
        if(key_map[i].key_io) 
        {
            ...
            else
            {
				...
				/*if(key_map[i].key_cnt >= Midd_Single_width )
				{
					if(power_keyup == 0)
                    {
				...
				else */ if((key_map[i].key_cnt >= Short_Single_width)/*&&(key_map[i].key_cnt < Midd_Single_width)*/) //Short_Single_width
                {
...
//    INT32U led_type;
	...
	DBG_PRINT("ap_peripheral_B_key_exe, tick_cnt_ptr = %d\r\n", *tick_cnt_ptr);
	if (!s_usbd_pin)
	{
	    ...
	    {			
			...
			{
				__msg("Send message to active video!, video_record_sts&0x02 = %d\n", video_record_sts&0x02);
			...
			else
			{
				__msg("Send message to active video!, video_record_sts&0x02 = %d\n", video_record_sts&0x02);
				msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
				msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_SET_MOT_D_STATUS, NULL, NULL, MSG_PRI_NORMAL);
			}
		}
	}
	else
...
//    INT32U led_type;
	...
	__msg("ap_peripheral_C_key_exe, tick_cnt_ptr = %d\n", *tick_cnt_ptr);
...
//    INT32U led_type;
	...
	__msg("ap_peripheral_D_key_exe, tick_cnt_ptr = %d\n", *tick_cnt_ptr);
	if (!s_usbd_pin)
	{
#if SUPPORT_LONGKEY == CUSTOM_ON
		if(*tick_cnt_ptr >= 64)
        {
			if ((ap_state_handling_storage_id_get() != NO_STORAGE)&& (!pic_down_flag) && (!video_down_flag))
		    {
		    	__msg("Send message to format TF !\n");
...
#endif	
		{
			if ((ap_state_handling_storage_id_get() != NO_STORAGE)&& (!pic_down_flag) && (!video_down_flag))
		    {
		    	__msg("Send message to move detect , video_record_sts&0x04 = %d!\n", video_record_sts& 0x04);
				if ((video_record_sts & 0x04) == 0)			//0x06 means move detect
		        {
					__msg("MSG_APQ_MD_SET !\n");
					msgQSend(ApQ, MSG_APQ_MD_SET, NULL, NULL, MSG_PRI_NORMAL);
		        }
				else //if(video_record_sts& 0x04 == 1)
				{
        ...
        if((ap_state_handling_storage_id_get() != NO_STORAGE) && (!pic_down_flag) && (!card_space_less_flag) && (!video_down_flag))
        {
            ...
            else
            {
                if(((video_record_sts & 0x02) == 0) )
                {
                    ...
                    {
                        current_state = STATE_AUDIO_RECORD;
                        msgQSend(ApQ, MSG_APQ_AUDO_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
                    }
                    else if(current_state == STATE_AUDIO_RECORD )
                    {
